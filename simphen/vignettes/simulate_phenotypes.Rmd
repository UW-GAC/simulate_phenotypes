...
eff <- variant_effect(geno, h2=0.01, varComp=varComp)
eff$beta
outcome.eff <- outcome + eff$Gbeta
```
...
geno <- as.vector(seqGetData(gds, "$dosage_alt"))
eff <- variant_effect(geno, beta=1.5, varComp=varComp)
eff$h2
outcome.eff <- outcome + eff$Gbeta
```
...
power(N=sum(afr), h2=eff.afr$h2, beta=eff.afr$beta) 
outcome.eff.afr <- outcome[afr] + eff.afr$Gbeta

eur <- sample.id %in% strata[["EUR"]]
eff.eur <- variant_effect(geno[eur], beta=1.5, varComp=varComp)
power(N=sum(eur), h2=eff.eur$h2, beta=eff.eur$beta) 
outcome.eff.eur <- outcome[eur] + eff.eur$Gbeta
```
