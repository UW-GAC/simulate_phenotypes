...
ggplot(dat.signif, aes(result, n)) + 
    ...
    facet_wrap(relative_freq~effect_size, scales="free_y") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```

```{r}
# dat.stat <- dat %>%
#     filter(!is.na(Score.Stat)) %>%
#     select(relative_freq, effect_size, variant.id, group, Score.Stat) %>%
#     pivot_wider(names_from=group, values_from=Score.Stat) %>%
...
p <- list()
for (g in unique(dat.stat$relative_freq)) {
p[[g]] <- dat.stat %>%
        filter(relative_freq %in% g) %>%
ggplot(aes(indiv, pooled)) +
    ...
    facet_grid(group~effect_size) +
    ggtitle(paste("relative freq:", g, 
                  "\n columns = relative effect size",
                  "\n rows = strata"))
}
